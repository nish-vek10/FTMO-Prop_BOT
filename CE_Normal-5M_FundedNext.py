"""
Chandelier Exit Strategy for XAUUSD — FundedNext-Style 2-Step Prop Rules (Demo)
...
risk_per_trade_pct_of_initial = 0.25   # % of initial account size risked per trade
sl_usd_distance                = 4.5    # fixed $ distance from entry (price units)

# === PARTIAL TP & BREAK-EVEN MGMT === #
# Two-tier partials for FundedNext:
tp1_usd_distance      = 4.5      # first target distance from entry (price units)
tp1_fraction          = 0.50     # close 50% at first target

tp2_usd_distance      = 8.5      # second target distance from entry (price units)
...
# (legacy single TP vars left commented to prevent accidental use elsewhere)
# tp_usd_distance     = 8.5
# partial_tp_fraction = 0.50



# === PROP RULES CONFIG (FundedNext-STYLE, 2-STEP) === #
...
# If True: send a "yesterday → now" FundedNext daily report + CSV ON STARTUP (useful for testing).
...
mt5_login = 52498280
mt5_password = "!d!4KwHHFIGmu9"
mt5_server = "ICMarketsSC-Demo"
mt5_terminal_path = r"C:\MT5\EA-FundedNext\terminal64.exe"
...
def _email_smoke_test():
    msg = EmailMessage()
    msg['Subject'] = 'SMTP test — PropEA - FundedNext'
...
def _maybe_manage_open_position():
    """
    Position management sequence (FundedNext spec):
      1) Take 50% at tp1_usd_distance (e.g., $4.5)
      2) AFTER (and only after) 50% is booked, move SL to BE+buffer (respect broker min distance)
      3) Take 25% at tp2_usd_distance (e.g., $8.5)
      4) Leave the runner; opposite signal elsewhere will flip/close it
    """
    ...
    # Evaluate touches (handle overshoot to TP2 by still doing TP1 first)
    ...
    # If the market jumps straight to TP2 before TP1, we handle TP1 first (above), then TP2.
    if tp2_hit:
